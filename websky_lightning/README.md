...
Full-page cache files do not expire or prune automatically. Product edits invalidate the matching product cache plus listing pages, while category edits invalidate the affected category and homepage cache; after a catalog write, the module warms replacement desktop, mobile, and tablet objects in the shutdown phase so visitors do not pay the first-request render cost. The admin **Cache all pages** action walks every product and category in small resumable batches and warms the safe desktop, mobile, and tablet variants. Browser capability headers do not create separate HTML page caches. Administrators can still clear every page explicitly with the module's clear-cache action.

Page-cache hits remain server-side only and return `private, no-store` HTML headers. This prevents a browser from replaying a cached guest header or zero cart after the visitor logs in or changes the cart. Authenticated, cart, checkout, and account requests remain excluded from shared full-page caching.
